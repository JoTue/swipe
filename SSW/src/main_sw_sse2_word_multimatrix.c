...
static void ssw_write2 (uint16_t score,
			const kseq_t* ref_seq,
			const kseq_t* read)
	// write function for ssw_sse2_byte output
{
	fprintf(stdout, "target_name: %s\nquery_name: %s\noptimal_alignment_score: %i\n\n", ref_seq->name.s, read->name.s, score);
}
...
int main (int argc, char * const argv[]) {
	...
	int16_t* mata = (int16_t*)calloc(25, sizeof(int16_t));
	const int16_t* mat = mata;
	char mat_name[16];
	mat_name[0] = '\0';
	...
	int8_t aa_table[128] = 
  {
	...
	// for matrix parsing
	FILE *matrix_fp = fopen(argv[optind + 2], "r");
	...
	while (kseq_read(read_seq) >= 0) {
		...
		while (kseq_read(ref_seq) >= 0) {
			...
			// parse matrix - NEW
			char line[1280];
			mata = (int16_t*)realloc(mata, 1024 * sizeof(int16_t));
			k = 0;
			m = 0;
			while (fgets(line, 1280, matrix_fp)) {
				if(line[0] == '#') {
					if (k>0) break;
				}
				if (line[0] == '*' || line[0] == '-' || (line[0] >= 'A' && line[0] <= 'Z')) {
					if (line[0] >= 'A' && line[0] <= 'Z') aa_table[(int)line[0]] = aa_table[(int)line[0] + 32] = m;
					char str[4], *s = str;
					str[0] = '\0';
					l = 1;
			...
			// fprintf(stderr, "n: %i\n", n);
			// fprintf(stderr, "aa_table:\n");
			// for (int b = 0; b < 128; b++){
			// 	fprintf(stderr, "%i", (int8_t)aa_table[b]);
			// 	fprintf(stderr, " ");
			// } 
			// fprintf(stderr, "\n");
			...
			// if (result->score1 >= filter) fprintf(stderr, "result->score1: %i\n", result->score1);
